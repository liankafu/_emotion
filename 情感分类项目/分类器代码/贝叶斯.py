...
def hyperparameter_tuning_naive_bayes(X_train, y_train):
    """朴素贝叶斯超参数调优"""
    print("\n" + "=" * 50)
    print("朴素贝叶斯超参数调优...")
    print("=" * 50)
...
def main():
    ...
    # 第一阶段：超参数调优
    print("\n" + "=" * 60)
    print("第一阶段：朴素贝叶斯超参数调优")
    print("=" * 60)

    # 朴素贝叶斯分类器超参数调优
    best_nb = hyperparameter_tuning_naive_bayes(X_train, y_train)
    ...
    # 使用调优后的朴素贝叶斯模型在验证集上评估
    y_pred_val = best_nb.predict(X_val)
    val_accuracy = accuracy_score(y_val, y_pred_val)

    print(f"朴素贝叶斯 验证集准确率: {val_accuracy:.4f}")
    ...
    # 在完整训练集上进行交叉验证
    cv_mean, cv_std = cross_validation_evaluation(best_nb, X_train_full, y_train_full, "朴素贝叶斯")
    ...
    # 使用完整训练集重新训练最佳模型
    print("使用完整训练集重新训练最佳朴素贝叶斯模型...")
    best_nb.fit(X_train_full, y_train_full)

    # 最终测试
    y_pred_final = best_nb.predict(X_test)
    ...
    print("-" * 75)
    print(f"{'朴素贝叶斯':<15} {val_accuracy:<15.4f} {cv_mean:<15.4f} {cv_std:<15.4f} {final_accuracy:<15.4f}")

    # 可选：保存最佳模型
    print("\n" + "=" * 50)
    save_model = input("是否保存最佳朴素贝叶斯模型？(y/n): ")
    if save_model.lower() == 'y':
        with open('best_nb_model_tuned.pkl', 'wb') as f:
            pickle.dump({
                'model': best_nb,
        ...
        print("调优后的朴素贝叶斯模型已保存为 'best_nb_model_tuned.pkl'")
