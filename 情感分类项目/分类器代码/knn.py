...
def main():
    ...
    # KNN分类器超参数调优
    best_knn = hyperparameter_tuning_knn(X_train_scaled, y_train)
    ...
    print(f"KNN 验证集准确率: {val_accuracy:.4f}")
    ...
    # 在完整训练集上进行交叉验证
    cv_mean, cv_std = cross_validation_evaluation(best_knn, X_train_full_scaled, y_train_full, "KNN")
    ...
    # 使用完整训练集重新训练最佳模型
    print("使用完整训练集重新训练最佳KNN模型...")
    best_knn.fit(X_train_full_scaled, y_train_full)
    ...
    print("-" * 75)
    print(f"{'KNN':<15} {val_accuracy:<15.4f} {cv_mean:<15.4f} {cv_std:<15.4f} {final_accuracy:<15.4f}")

    # 可选：保存最佳模型
    print("\n" + "=" * 50)
    save_model = input("是否保存最佳KNN模型？(y/n): ")
    if save_model.lower() == 'y':
        ...
        print("调优后的KNN模型已保存为 'best_knn_model_tuned.pkl'")
